...
describe('install-agent-status.mjs', () => {
  it('合并 Codex/Claude hooks、保留原配置，并可重复执行', () => {
    ...
    expect(codex.hooks.PermissionRequest[0].hooks[0].command).toContain('codex waiting')
    expect(codex.hooks.Stop).toHaveLength(1)
    expect(claude.theme).toBe('dark')
    expect(claude.hooks.SessionStart).toHaveLength(1)
    expect(claude.hooks.PermissionRequest[0].hooks[0].command).toContain('claude waiting')
    expect(claude.hooks.Elicitation[0].hooks[0].command).toContain('claude waiting')
    expect(claude.hooks.ElicitationResult[0].hooks[0].command).toContain('claude running')
