...
- **Codex**：用 `~/.codex/auth.json` 里的 OAuth token 调 ChatGPT 的 usage
  接口，拿当前百分比、重置倒计时和计划档位。打开它即表示同意读取该凭据
  文件并向 `chatgpt.com` 发起 HTTPS 请求。
- **Claude Code**：Claude
...
每条配额进度条上有一根红色基准线，画在「窗口已过去的比例」处——5 小时窗口
还剩 1.5 小时就在 70%。用量超过这条线，说明烧得比时间快，窗口重置前会先
耗光。
