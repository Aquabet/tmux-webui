...
| Amber, steady | `#e0af68` | **Waiting:** the agent is waiting for user input |
| No dot | — | The agent stopped, its exact state is unknown, or this is a Terminal fallback |
...
| Bright | Amber | Someone is viewing the session and the agent is waiting for user input |
| Dim gray | Amber | Nobody is viewing the session and the agent is waiting for user input |
| Bright or dim | No dot | The agent stopped or its exact work status is unavailable |
...
**waiting for user input** when every detected pane of that kind has explicitly
reported idle. `idle` remains the legacy hook value and is normalized to
`waiting` in the API.
